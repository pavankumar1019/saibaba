...
class crud // la class des operations avec la base de données.
{
	...
	public function create($fname,$lname,$email,$contact,$event,$event_date) // methode d'insertion des données.
	{
		try
		{
			// préparation de la requete :
			$stmt = $this->db->prepare(
				"INSERT INTO tbl_Devotees_Data(first_name,last_name,email_id,contact_no,event_t,event_date) 
						VALUES(:fname, :lname, :email, :contact, :event, :event_date)");
	...
	{
		$stmt = $this->db->prepare("SELECT * FROM tbl_Devotees_Data WHERE id=:id"); // preparation de la requete sql avec l'id.
	...
	{
		$stmt = $this->db->prepare("DELETE FROM tbl_Devotees_Data WHERE id=:id"); // préparation.
